...
func (s *Service) CreateChannel(user authctx.CurrentUser, input CreateChannelInput) (ChannelSummary, error) {
	...
	err = s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := createSystemDefaultCollections(tx, channel, user.ID); err != nil {
			return err
		}
...
func createSystemDefaultCollections(tx *gorm.DB, channel model.Channel, ownerID uuid.UUID) error {
	collection := model.ContentCollection{
