...
const debateWithVoteCountSelect = `debates.*,
	(SELECT COUNT(*)
	 FROM debate_votes
	 JOIN comment_entries ON comment_entries.id = debate_votes.argument_id
	 JOIN discussion_targets ON discussion_targets.id = comment_entries.target_id
	 WHERE discussion_targets.kind = 'debate'
	   AND discussion_targets.resource_id = debates.id
	   AND comment_entries.deleted_at IS NULL
