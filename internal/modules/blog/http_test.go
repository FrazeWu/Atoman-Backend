...
func newBlogHTTPTestService(t *testing.T) (*Service, *gorm.DB, authctx.CurrentUser) {
	...
	testdb.Migrate(t, db,
		...
		&model.BlogRecommendationFeedback{},
		&model.Bookmark{},
...
func TestLoadCanonicalBlogContentsUsesCanonicalRuntimeType(t *testing.T) {
	...
	posts, err := LoadCanonicalBlogPosts(db, CanonicalBlogPostsQuery(db).Where("posts.id = ?", post.ID))
	if err != nil {
		t.Fatalf("load legacy-compatible posts: %v", err)
	}
	if len(posts) != 1 || posts[0].ID != post.ID || posts[0].Collection == nil || posts[0].Collection.ID != collection.ID {
		t.Fatalf("unexpected legacy-compatible projection: %#v", posts)
	}
	var legacyCount int64
	if err := db.Model(&model.Post{}).Where("id = ?", post.ID).Count(&legacyCount).Error; err != nil {
		t.Fatalf("count legacy posts: %v", err)
