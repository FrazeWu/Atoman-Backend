...
func TestRegisterRoutesAlbumDetailMissingAlbumReturns404(t *testing.T) {
	service, _, user := newMusicHTTPTestService(t)
	response := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/api/v1/music/albums/01a022d6-8a65-74a3-b951-dc72095777af", nil)
	newMusicHTTPRouter(service, &user).ServeHTTP(response, request)
