...
func buildAlbumImportDTO(session model.AlbumImportSession) AlbumImportDTO {
	...
	dto := AlbumImportDTO{
		...
		MetadataExternalID:      stringValue(payload["metadata_external_id"]),
	...
	if rawTracks, ok := payload["derived_tracks"].([]any); ok {
		for _, rawTrack := range rawTracks {
			...
			track := AlbumImportDTOTrack{
				...
				FileID:          stringValue(trackMap["file_id"]),
				...
				MatchStatus:     stringValue(trackMap["match_status"]),
				MatchProvider:   stringValue(trackMap["match_provider"]),
				MatchExternalID: stringValue(trackMap["match_external_id"]),
				MatchSourceURL:  stringValue(trackMap["match_source_url"]),
