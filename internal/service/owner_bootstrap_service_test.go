...
func TestEnsureOwnerCreatesDefaultResources(t *testing.T) {
	...
	var channels int64
	if err := db.Model(&model.Channel{}).Where("user_id = ?", owner.UUID).Count(&channels).Error; err != nil || channels != 0 {
		t.Fatalf("expected owner creation to defer channel creation, got %d err=%v", channels, err)
	}
	var collections int64
	if err := db.Model(&model.ContentCollection{}).Where("created_by = ?", owner.UUID).Count(&collections).Error; err != nil || collections != 0 {
