...
func TestRunFeedItemUniqueIndexCreatesExpectedIndex(t *testing.T) {
	...
	assertIndexExists(t, db, "feed_items", "idx_feed_items_source_link")
}
...
func TestRunFeedItemUniqueIndexDeduplicatesExistingRows(t *testing.T) {
	...
	if err := db.Exec(`DROP INDEX IF EXISTS idx_feed_items_source_link`).Error; err != nil {
		t.Fatalf("drop preexisting link unique index: %v", err)
	}
...
func TestRunFeedItemUniqueIndexDeduplicatesExistingRowsByLink(t *testing.T) {
	db := testdb.Open(t)
	testdb.Migrate(t, db, &model.FeedSource{}, &model.FeedItem{})
	if err := db.Exec(`DROP INDEX IF EXISTS idx_feed_items_source_guid`).Error; err != nil {
		t.Fatalf("drop preexisting guid unique index: %v", err)
	}
	if err := db.Exec(`DROP INDEX IF EXISTS idx_feed_items_source_link`).Error; err != nil {
		t.Fatalf("drop preexisting link unique index: %v", err)
	}
	...
	source := model.FeedSource{
		Base:       model.Base{ID: uuid.MustParse("018f0f58-31d2-7e35-bf72-444444444444")},
		SourceType: "external_rss",
		Hash:       "feed-item-link-dedupe-source",
		RssURL:     "https://example.com/feed.xml",
	}
	if err := db.Create(&source).Error; err != nil {
		t.Fatalf("create source: %v", err)
	...
	oldID := uuid.MustParse("018f0f58-31d2-7e35-bf72-555555555555")
	newID := uuid.MustParse("018f0f58-31d2-7e35-bf72-666666666666")
	oldCreatedAt := time.Date(2026, 6, 1, 8, 0, 0, 0, time.UTC)
	newCreatedAt := oldCreatedAt.Add(time.Hour)
...
INSERT INTO feed_items (
	id, created_at, updated_at, feed_source_id, guid, title, link, published_at, fetched_at, full_text_status, full_text_attempt_count, full_text_word_count
		...
		oldID, oldCreatedAt, oldCreatedAt, source.ID, "post-old-guid", "Older", "https://example.com/same-link", oldCreatedAt, oldCreatedAt, "disabled", 0, 0,
		newID, newCreatedAt, newCreatedAt, source.ID, "post-new-guid", "Newer", "https://example.com/same-link", newCreatedAt, newCreatedAt, "disabled", 0, 0,
	).Error; err != nil {
		t.Fatalf("seed duplicate feed items: %v", err)
	}
